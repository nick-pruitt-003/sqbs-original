...
- (void)windowControllerDidLoadNib:(NSWindowController *)aController
{
    ...
    [self setMinRoundIncludedInReport:1];
    [self setMaxRoundIncludedInReport:1];
...
- (BOOL)loadTournamentFile:(NSString *)fileName {
    ...
    if (data != nil) {
            ...
            if ((memberCount > maxRecordsPerTeam) && !suppressAlert) {
                ...
                NSAlert *alert = [NSAlert alertWithMessageText:@"Current max players per team exceeded." 
                                               ...
                                               alternateButton:@"Abort" 
                                                   otherButton:nil 
                                     informativeTextWithFormat:[NSString stringWithFormat:@"Team '%@'. Only first %i players imported.", [lines objectAtIndex:lineIndex], kMaxPlayersPerTeam]];
        ...
        //DebugLog(@"loadTournament - report: {%i, %i}, assign: {%i, %i,}", minRoundIncludedInReport, maxRoundIncludedInReport, minRoundsAssigned, maxRoundsAssigned);
...
- (NSString *)createFileRecordForTeam:(long)teamIndex {
    ...
    NSString *record = [NSString stringWithFormat:@"%@%i%@%@", kLineEndCharacter, recordCount, kLineEndCharacter, [theTeam objectForKey:kTeamNameKey]];
    recordCount = recordCount - 1;
...
- (BOOL)saveTournamentFile:(BOOL)fileTypeIsOSX {
    ...
    NSString *teamCountRecord = [NSString stringWithFormat:@"%@%i", kLineEndCharacter, teamCount];
    [data appendFormat:@"%i", teamCount];  // first record so no line end
                                           // add team name info
    ...
    [data appendFormat:@"%@%i", kLineEndCharacter, gameCount];
    ...
    [data appendFormat:@"%@%i", kLineEndCharacter, tempAutoTrackSetting];
    // for SQBS vers 2
    ...
    [data appendFormat:@"%@%i", kLineEndCharacter, (trackPowerNegStatsSetting + 2)];
    [data appendFormat:@"%@%i", kLineEndCharacter, trackLightRoundSetting];
    [data appendFormat:@"%@%i", kLineEndCharacter, trackTossUpsHeardSetting];
    // for SQBS vers 2
    ...
    [data appendFormat:@"%@%i", kLineEndCharacter, (tossUpHeardSortSetting + 2)];
    [data appendFormat:@"%@%i", kLineEndCharacter, [self warningsAsInt]];
    ...
    // add sort method
    [data appendFormat:@"%@%i", kLineEndCharacter, sortMethodSetting];     
    ...
    // add path setting
    [data appendFormat:@"%@%i", kLineEndCharacter, [self makePathsValue]]; 
    ...
    [data appendFormat:@"%@%i", kLineEndCharacter, divisionCount];
    ...
    for (int i = 0; i < teamCount; i++) {
        ...
        [data appendFormat:@"%@%i", kLineEndCharacter, divisionIndex];
    }
    
    // add question values
    [data appendFormat:@"%@%i", kLineEndCharacter, question_0_value];
    [data appendFormat:@"%@%i", kLineEndCharacter, question_1_value];
    [data appendFormat:@"%@%i", kLineEndCharacter, question_2_value];
    [data appendFormat:@"%@%i", kLineEndCharacter, question_3_value];
    ...
    if (packetNamesUsed > 0) {
        ...
        [data appendFormat:@"%@%i", kLineEndCharacter, (maxRoundsAssigned - minRoundsAssigned + 1)];
        for (double roundNumber = minRoundsAssigned; roundNumber <= maxRoundsAssigned; roundNumber++) {
            NSString *name = [packetNames objectForKey:[NSString stringWithFormat:kPacketRoundBaseKeyFormat, roundNumber]];
    ...
    for (int i = 0; i < teamCount; i++) {
        ...
        [data appendFormat:@"%@%i", kLineEndCharacter, exhibition];
    }
    ...
    if (data != nil) {
        ...
        if ((numberOfTeams <= 0) || (numberOfTeams > 100)) {
            ...
            NSAlert *alert = [NSAlert alertWithMessageText:@"File contents suspect. May not be a team file." 
                                           ...
                                           alternateButton:nil 
                                               otherButton:nil 
                                 informativeTextWithFormat:[NSString stringWithFormat:@"Team count read as \"%@\". Expected number [1 - 100].", firstLine]];
            [alert runModal];
            alert = nil;
            data = nil;
            keys = nil;
...
- (void)getMergeDidEnd:(NSWindow *)mergePanel returnCode: (int)code contextInfo: (void *)context {
    ...
    for (Tournament *mergingTournament in openDocumentsForMergeOrPublish) {
        ...
        if (proceedWithMerge) {
            ...
            for (double roundNumber = minRoundsAssigned; roundNumber <= maxRoundsAssigned; roundNumber++) {
                NSString *packetName = [packetNames objectForKey:[NSString stringWithFormat:kPacketRoundBaseKeyFormat, roundNumber]];
                if ((packetName == nil) || [packetName isEqualToString:kPacketMissingPacketNameValue]) {
                    ...
                    NSString *mergePacketName = [[mergingTournament packetNames] objectForKey:[NSString stringWithFormat:kPacketRoundBaseKeyFormat, roundNumber]];
                    if (mergePacketName != nil) {
                        // copy even if missing packet name as this might be round that doesn't exist in base tournament
                        [packetNames setObject:mergePacketName forKey:[NSString stringWithFormat:kPacketRoundBaseKeyFormat, roundNumber]];
                    }
                }
            }
...
- (void)adjustTeamIndexesInGames {
    ...
    for (int gID = 0; gID < [gameList count]; gID++) {
        ...
        if ([theGame team_A_index] >= 0) {
            indexKey = [NSString stringWithFormat:@"%i", [theGame team_A_index]];
            [theGame setTeam_A_index:[(NSString *)[map objectForKey:indexKey] intValue]];
        }
        if ([theGame team_B_index] >= 0) {
            indexKey = [NSString stringWithFormat:@"%i", [theGame team_B_index]];
        ...
        if (allRoundsIncludedInReport || (((rIndex + minRoundsAssigned) >= minRoundIncludedInReport) && ((rIndex + minRoundsAssigned) <= maxRoundIncludedInReport))) {
            ...
            if ([thisRound gamesPlayed] > 0) {
                ...
                if ((roundName == nil) || [roundName isEqualToString:kPacketMissingPacketNameValue]) {
                    roundName = [NSString stringWithFormat:@"%i", (rIndex + minRoundsAssigned)];
                }
...
- (void)teamDetailReportCreate {
    ...
    for (NSMutableDictionary *theTeam in teamList) {
        ...
        if (question_0_value != 0) {
            [content appendFormat:@"  <td ALIGN=RIGHT><B>%i</B></td>\n", question_0_value];
        }
                ...
                if (([thisGame team_A_index] == teamIndex) && ([thisGame team_B_index] >= 0)) {
                    ...
                    if ([thisGame forfeit]) {
                        ...
                        if (roundReportSetting && (packetNamesUsed > 0)) {
                            ...
                            if ((roundName == nil) || [roundName isEqualToString:kPacketMissingPacketNameValue]) {
                                roundName = [NSString stringWithFormat:@"%i", [thisGame round]];
                            }
        ...
        for (Individuals *thisPlayer in individualStandings) {
            if ([[thisPlayer teamName] isEqualToString:teamName] && ([thisPlayer gamesPlayed] > 0)) {
                content = [NSMutableString stringWithString:@"<tr>\n  <td ALIGN=LEFT>"];
                if (individualStandingsReportSetting) {
                    [content appendFormat:@"<A HREF=%@%@#p%i_%i>",baseName, individualDetailsReportName, [thisPlayer position], [thisPlayer teamIndex]];
                }
...
- (void)scoreboardReportCreate {
    ...
    if ([sortedGameList count] > 0) {
        ...
        // first round header
        if (roundReportSetting) {
            [content appendFormat:@"<FONT SIZE=+1 COLOR=red>Round %i", currentRound];
            if (packetNamesUsed > 0) {
            ...
            if (allRoundsIncludedInReport || (([thisGame round] >= minRoundIncludedInReport) && ([thisGame round] <= maxRoundIncludedInReport))) {
                if (([thisGame team_A_index] >= 0) && ([thisGame team_B_index] >= 0)) {
                    if (roundReportSetting && ([thisGame round] > currentRound)) {
                        ...
                        [content appendFormat:@"<hr><FONT SIZE=+1 COLOR=red>Round %i", currentRound];
                        if (packetNamesUsed > 0) {
                            NSString *packetName = [packetNames objectForKey:[NSString stringWithFormat:kPacketRoundBaseKeyFormat, currentRound]];
                            if ((packetName != nil) && ![packetName isEqualToString:kPacketMissingPacketNameValue]) {
                                [content appendFormat:@" (Packet: %@)", packetName];
                    ...
                    if ([thisGame forfeit]) {
                        ...
                        if ((teamAscore != -1) && (teamBscore != -1)) {
                            if (teamAscore > teamBscore) {
                                [content appendFormat:@"<FONT SIZE=+1>%@ %i, %@ %i", teamAname, teamAscore, teamBname, teamBscore];
                            } else {
                                [content appendFormat:@"<FONT SIZE=+1>%@ %i, %@ %i", teamBname, teamBscore, teamAname, teamAscore];
                            }
                            if (teamAscore == teamBscore) {
            ...
            if (!useDivisionsSetting || [divisionName isEqualToString:[(NSMutableDictionary *)[teamList objectAtIndex:[thisTeam teamIndex]] objectForKey:kDivisionKey]]) {
                ...
                if (gp > 0) {
                    ...
                    if (teamDetailsSetting) {
                        [content appendFormat:@"<A HREF=%@%@#t%i>", baseName, teamDetailsReportName, [thisTeam teamIndex]];
                    }
...
- (void)statKeyReportCreate {
    ...
    [content appendString:@"<tr>\n<td>"];
    if (question_0_value != 0) { [content appendFormat:@"%i ", question_0_value]; }
    if (question_1_value != 0) { [content appendFormat:@"%i ", question_1_value]; }
    if (question_2_value != 0) { [content appendFormat:@"%i ", question_2_value]; }
    if (question_3_value != 0) { [content appendFormat:@"%i ", question_3_value]; }
    if (britishStyleReportSetting) {
...
- (IBAction)createWebReports:(id)sender {
    ...
    if ([saveDlg runModal] == NSFileHandlingPanelOKButton)
    {
        ...
        NSString *qValues = [NSString stringWithFormat:@"%i/%i/%i/%i", question_0_value, question_1_value, question_2_value, question_3_value];
...
- (IBAction)quickPrintGameReport:(id)sender {
    ...
    if ([sortedGameList count] > 0) {
        ...
        // first round header
        if (roundReportSetting) {
            [content appendFormat:@"<tr>\n  <td></td>\n</tr>\n<tr>\n  <td colspan=4><B>Round %i", currentRound];
            if (packetNamesUsed > 0) {
                NSString *packetName = [packetNames objectForKey:[NSString stringWithFormat:kPacketRoundBaseKeyFormat, currentRound]];
                    ...
                    if ([thisGame forfeit]) {
                        ...
                        if ((teamAscore != -1) && (teamBscore != -1)) {
                            if (teamAscore > teamBscore) {
                                ...
                                [content appendFormat:@"%i", teamAscore];
                                [content appendString:@"  <td style=\"width:30%\">\n"];
                                [content appendString:teamBname];
                                [content appendString:@"  <td style=\"width:15%\">\n"];
                                [content appendFormat:@"%i", teamBscore];
                            } else {
                                [content appendString:@"  <td style=\"width:30%\">\n"];
                                [content appendString:teamBname];
                                [content appendString:@"  <td style=\"width:15%\">\n"];
...
#pragma mark -
#pragma mark Server methods & actions

//- (void) getServerIdentityDidEnd:(NSWindow *)panel returnCode:(int)returnCode contextInfo:(NSMenuItem *)context {
//    [panel close];
//    [self setTemporaryWindowController:nil];
//    if (returnCode == NSAlertAlternateReturn) {
//        // user cancelled, nothing more
//        return;
//    }
//    // start server
//    if (![(TournamentAppDelegate *)[NSApp delegate] startServer]) {
//        //
//        NSAlert *alert = [NSAlert alertWithMessageText:@"SQBS Server failed to start" 
//                                         defaultButton:nil 
//                                       alternateButton:nil 
//                                           otherButton:nil 
//                             informativeTextWithFormat:@""];
...
//    }
//    
//    [context setTitle:@"Stop Server"];
//}
//
//
//- (IBAction)startStopServer:(id)sender {
//    if ([[(NSMenuItem *)sender title] isEqualToString:@"Start Server"]) {
//        // get server identity
//        ServerIdentityWindowController *temp = [[ServerIdentityWindowController alloc] initWithWindowNibName:@"ServerIdentityWindow"];
//        [self  setTemporaryWindowController:temp];
//        [temp release];
//        [NSApp beginSheet:[temporaryWindowController window] modalForWindow:[viewSelectorSegmentedControl window] modalDelegate:self didEndSelector:@selector(getServerIdentityDidEnd:returnCode:contextInfo:) contextInfo:sender];
//    } else {
//        if (![(TournamentAppDelegate *)[NSApp delegate] stopServer]) {
//            //
//            NSAlert *alert = [NSAlert alertWithMessageText:@"SQBS Server failed to stop" 
//                                             defaultButton:nil 
//                                           alternateButton:nil 
//                                               otherButton:nil 
//                                 informativeTextWithFormat:@""];
...
//            return;
//        }
//        [(NSMenuItem *)sender setTitle:@"Start Server"];
//    }
//}
//
//- (void)getPublishDidEnd:(NSWindow *)publishPanel returnCode: (int)code contextInfo: (void *)context {
//    [publishPanel close];
//    if (code == NSAlertAlternateReturn) {
//        // cancel
//        [self setTemporaryWindowController:nil];
...
//        return;
//    }
//    NSIndexSet *selectedDocs = [(PublishWindowController *)temporaryWindowController selectedDocumentsIndexSet];
//    for (int i = 0; i < [openDocumentsForMergeOrPublish  count]; i++) {
//        if ([selectedDocs containsIndex:i]) {
//            Tournament *tournamentToPublish = [openDocumentsForMergeOrPublish objectAtIndex:i];
//            PublishedTournament *publishTournament = [[PublishedTournament alloc] init];
//            
//            [publishTournament setTournamentName:[tournamentToPublish tournamentName]];
//            [publishTournament setTeamList:[tournamentToPublish teamList]];
//            NSMutableArray *questionValues = [NSMutableArray arrayWithCapacity:4];
//            // assign question values, if not used assign large number as indication
//            int value = [tournamentToPublish question_0_selected]? [tournamentToPublish question_0_value] : 1000;
//            [questionValues addObject:[NSNumber numberWithInt:value]];
//            value = [tournamentToPublish question_1_selected]? [tournamentToPublish question_1_value] : 1000;
//            [questionValues addObject:[NSNumber numberWithInt:value]];
//            value = [tournamentToPublish question_2_selected]? [tournamentToPublish question_2_value] : 1000;
//            [questionValues addObject:[NSNumber numberWithInt:value]];
//            value = [tournamentToPublish question_3_selected]? [tournamentToPublish question_3_value] : 1000;
//            [questionValues addObject:[NSNumber numberWithInt:value]];
//            [publishTournament setQuestionValues:questionValues];
//            questionValues = nil;
//            
//            [(TournamentAppDelegate *)[NSApp delegate] addTournamentForPublish:publishTournament];
//            
//            [publishTournament release];
//        }
//    }
//    
//    [self setOpenDocumentsForMergeOrPublish:nil];
...
//    NSArray *openDocuments = [[NSDocumentController sharedDocumentController] documents];
//    
//    PublishWindowController *temp = [[PublishWindowController alloc] initWithWindowNibName:@"PublishWindow"];
//    NSMutableArray *docs = [NSMutableArray arrayWithCapacity:3];
//    for (Tournament *thisTournament in openDocuments) {
//        // tournaments must be named
//        NSString *tName = [[thisTournament tournamentName] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
//        
//        if ((tName == nil) || ([tName isEqualToString:@""])) {
//            tName = [NSString stringWithFormat:@"Tournament %@", [NSDateFormatter localizedStringFromDate:[NSDate date] dateStyle:NSDateFormatterMediumStyle timeStyle:NSDateFormatterLongStyle]];
//            [thisTournament setTournamentName:tName];
//            [thisTournament handleTournamentNameUpdate];
//            [thisTournament updateChangeCount:NSChangeDone];
//            [[thisTournament myTargetView] setNeedsDisplay:YES];
//        }
//        [docs addObject:thisTournament];
//    }
//    [temp setDocumentsArray:docs];
//    [self setOpenDocumentsForMergeOrPublish:docs];
//    [self setTemporaryWindowController:temp];
//    [temp release];
//    docs = nil;
//    [NSApp beginSheet:[temporaryWindowController window] modalForWindow:[viewSelectorSegmentedControl window] modalDelegate:self didEndSelector:@selector(getPublishDidEnd:returnCode:contextInfo:) contextInfo:nil];
//}
//
//- (IBAction)unpublishTournament:(id)sender {
//    DebugLog(@"unpublishTournament");
//    // no actiion done, here so validateItem works correctly
//}
//
...
//    while ([[(TournamentAppDelegate *)[NSApp delegate] publishedTournaments] count] > 0) {
//        [(TournamentAppDelegate *)[NSApp delegate] removeTournamentForPublish:[[(TournamentAppDelegate *)[NSApp delegate] publishedTournaments] objectAtIndex:0]];
//    }
//}
//
//- (IBAction)importAllGames:(id)sender {
//    DebugLog(@"importAllGames");
//}
//
//- (void)handleTournamentNameUpdate {
//    if ([viewSelectorSegmentedControl selectedSegment] == kTournamentOptionsView) {
//        [[(TournamentSetupViewController *)myCurrentViewController tournamentNameTextField] setStringValue:[self tournamentName]];
//    }
//}
//
...
//    @try {
//        theGameFile = [NSKeyedUnarchiver unarchiveObjectWithFile:fileName];
//    }
//    @catch (NSException *exception) {
//        theGameFile = nil;
//    }
//    if (theGameFile != nil) {
//        // compile data for data for each player - points scored & tossups present
//    }
//}
