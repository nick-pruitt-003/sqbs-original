...
- (void)loadSettingsFromDelegate {
    ...
    long value = [documentDelegate minRoundsAssigned];
    [minRoundIncludedTextField setIntegerValue:value];
    [[maxRoundIncludedTextField formatter] setMinimum:[NSNumber numberWithDouble:value]];
    [[maxRoundIncludedTextField formatter] setMaximum:[NSNumber numberWithDouble:[documentDelegate maxRoundsAssigned]]];
    [maxRoundIncludedTextField setIntegerValue:[documentDelegate maxRoundsAssigned]];
    ...
    for (long roundNumber = minRoundNumber; roundNumber <= maxRoundNumber; roundNumber++) {
        ...
        NSMutableDictionary *aRound = [NSMutableDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"%i", roundNumber], kPacketSettingsRoundKey, thePacketName, kPacketSettingsPacketNameKey, nil];
        //DebugLog(@"loadSettingsFromDelegate packets - index: %i, dictionary: %@", roundNumber, aRound);
...
-(void)savePacketNames {
    ...
    for (NSMutableDictionary *theRoundDict in packetNameArray) {
        ...
        [newPacketNameDictionary setObject:packetName forKey:[NSString stringWithFormat:kPacketRoundBaseKeyFormat, [[theRoundDict objectForKey:kPacketSettingsRoundKey] intValue]]];
        packetName = nil;
    }
...
- (IBAction)roundsMinMaxChangedAction:(id)sender {
    ...
    if (sender == minRoundIncludedTextField) {
        if (value == [minRoundIncludedTextField integerValue]) {
            return;
        }
