...
-(NSString *)getTeamAQuestionPoints {
    ...
    return [NSString stringWithFormat:@"%li/%li/%li/%li", q0Total, q1Total, q2Total, q3Total];
}
    ...
    for (int pIndex = 0; pIndex < [team_A_playerDetails count]; pIndex++) {
        if ([[team_A_playerDetails objectAtIndex:pIndex] isKindOfClass:[Player class]]) {
            ...
            if ([aPlayer playerIndex] == thePlayerNumber) {
                ...
                return [NSString stringWithFormat:@"%.2f/%li/%li/%li/%li/%li", gamesPlayed, [aPlayer question0Count], [aPlayer question1Count], [aPlayer question2Count], [aPlayer question3Count], [aPlayer points]];
            }
        }
...
-(NSString *)getTeamAResultsForPlayerAtIndex:(long)arrayIndex {
    ...
    if ([[team_A_playerDetails objectAtIndex:arrayIndex] isKindOfClass:[Player class]]) {
        ...
        return [NSString stringWithFormat:@"%.2f/%li/%li/%li/%li/%li/%li", gamesPlayed, [aPlayer question0Count], [aPlayer question1Count], [aPlayer question2Count], [aPlayer question3Count], [aPlayer points],[aPlayer playerIndex]];
    }
    return @"0/0/0/0/0/0/-1";
}
...
-(NSString *)createGameRecord {
    NSMutableString *record = [NSMutableString stringWithFormat:@"%@%@%@%li%@%li%@%@%@%@", kLineEndCharacter, gameIndex,
                               kLineEndCharacter, team_A_index,
                               kLineEndCharacter, team_B_index,
                               kLineEndCharacter, team_A_score,
                               kLineEndCharacter, team_B_score];
    
    [record appendFormat:@"%@%li%@%li%@%li%@%li%@%li%@%li", kLineEndCharacter, tossUpsHeard,
    ...
    [record appendFormat:@"%@%li%@%li%@%li%@%li%@%li%@%li", kLineEndCharacter, overtime,
...
-(NSString *)description {
    NSMutableString *record = [NSMutableString stringWithFormat:@"Game: %@, Teams: {%li, %li}, Scores: {%@, %@}", gameIndex, team_A_index, team_B_index, team_A_score, team_B_score];
    if (team_A_playerDetails != nil) {
        [record appendString:@"\nTeam A players:\n"];
        for (int p = 0; p < 8; p++) {
            // add Team A player p info
